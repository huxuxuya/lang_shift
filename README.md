...
После того как вы найдёте имя этой переменной, в файле `lang_shift\include.h` нужно написать:
...
А для шифта и переключения языка используете кейкоды `SFT_N`, `LA_CHNG`, etc. Остальные смотите в файле `lang_shift\include.h`.

Для английского и русского языка используйте кейкоды с префиксами `EN_*`, `RU_*`, все их посмотреть можно в файле `lang_shift\include.h`.

Для модификаторов используйте клавиши `CTRL_0`, `ALT_0`, etc. Остальные смотите в файле `lang_shift\include.h`.
